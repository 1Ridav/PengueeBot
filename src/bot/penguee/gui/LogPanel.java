...
public class LogPanel extends JPanel {
	...
	public LogPanel(JFrame frame) {
		...
		labelStop.setIcon(new ImageIcon(LogPanel.class
				.getResource("/res/stop.png")));
		...
		labelRun = new MyJLabel("RUN");
		labelRun.setVisible(true);
		labelRun.addMouseListener(new MouseAdapter() {

			@Override
			public void mouseClicked(MouseEvent arg0) {
				runScript(Data.scriptFileName);
			}
		});
		labelRun.setIcon(new ImageIcon(LogPanel.class
		...
		JLabel lblNewLabel = new MyJLabel("LOAD");
		lblNewLabel.setIcon(new ImageIcon(LogPanel.class
		...
		lblNewLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (f != null && d != null) {
					Data.scriptFileName = d+f;
					currentScriptNameLabel.setText(Data.scriptFileName);
				}
			}
		});
	...
	public void runScript(String name) {
		try {
			...
			params.add("java");
			params.add("-Xmx" + Data.xmxValue + "M");
			...
			IOredirect.redirectOutput(logTextArea,
					scriptProcess.getInputStream(),
					scriptProcess.getErrorStream());
			//new thread to wait until process will not quit with ok status, then change buttons state
